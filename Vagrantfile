Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    echo '\$TTL    7200' | sudo tee /etc/bind/db.sistema.test
    ...
    echo '            604800 ) ; Negative Cache TTL' | sudo tee -a /etc/bind/db.192
    ...
    echo 'listen-on { any; };' | sudo tee -a /etc/bind/named.conf.options
  ...
  config.vm.define "venus" do |venus|
    ...
    venus.vm.provision "shell", inline: <<-SHELL
      ...
      echo '    type slave;' | sudo tee -a /etc/bind/named.conf.local
      echo '    masters { 192.168.57.103; };' | sudo tee -a /etc/bind/named.conf.local
      echo '    file "/var/cache/bind/db.192";' | sudo tee -a /etc/bind/named.conf.local
      echo '};' | sudo tee -a /etc/bind/named.conf.local
